...
Mesh::Mesh(const char* fileName, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
{
	// File input object
	...
	std::vector<DWORD> indices;           // Indices of these verts
	...
	CreateBuffers(&verts[0], vertCounter, &indices[0], indexCount, device, commandList);
}


Mesh::Mesh(Vertex* vertices, int vertexCount, DWORD* indices, int indexCount, ID3D12Device* device, ID3D12GraphicsCommandList* commandList) : indexCount(indexCount)
{
	CreateBuffers(vertices, vertexCount, indices, indexCount, device, commandList);
}
...
void Mesh::CreateBuffers(Vertex* vertices, int vertexCount, DWORD* indices, int indexCount, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
{
	HRESULT hr;
	...
	// give resource heaps a name so when we debug with the graphics debugger we know what resource we are looking at
	...
	size_t indexBufferSize = indexCount * sizeof(DWORD);
